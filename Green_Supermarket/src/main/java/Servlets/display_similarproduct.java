...
@WebServlet(name = "display_similarproduct", value = "/display_similarproduct")
public class display_similarproduct extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = dbconnection.getConnection()) {
            String sql = "SELECT product_id, product_name, product_category, product_details, product_price, product_image_path FROM green_sp_db.product_table WHERE product_category = ? AND product_id <> ? LIMIT 3";
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                ...
                pstmt.setString(2, currentProductId);

                try (ResultSet rs = pstmt.executeQuery()) {
                    int cardCount = 0;

                    while (rs.next() && cardCount < 3) {
            ...
            e.printStackTrace();
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Error fetching similar product data");
